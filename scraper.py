#!/usr/bin/env python3
"""
Vita ⴾ — منشئ ملفات المشروع الحيوي (النسخة النهائية الكاملة)
يقوم بإنشاء جميع ملفات المشروع في نفس المجلد الذي يعمل منه
"""
import os

def create_file(path, content):
    """إنشاء ملف وكتابة المحتوى داخله"""
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body {
            ...
            background: radial-gradient(ellipse at center, #0a1a0a 0%, #020502 100%);
            color: #c8e6c9;
            min-height: 100vh;
            display: flex;
        ...
        .splash-logo {
            font-size: 5em; animation: pulse 1.5s infinite;
            color: #4caf50; text-shadow: 0 0 40px rgba(76,175,80,0.7);
        }
        .splash-text { color: #81c784; margin: 20px; font-size: 1.2em; }
        .splash-bar {
            width: 200px; height: 4px;
            background: #1a3a1a; border-radius: 4px; overflow: hidden;
        }
        .splash-fill {
            width: 0%; height: 100%;
            background: #4caf50;
        ...
        @keyframes float {
            ...
            50% { transform: translateY(-60px) translateX(-20px) scale(1); }
            75% { transform: translateY(-30px) translateX(-30px) scale(0.8); }
        }
        ...
        .logo-main {
            font-size: 6em; animation: pulse 2s infinite;
            color: #4caf50; text-shadow: 0 0 40px rgba(76,175,80,0.7);
            cursor: pointer; user-select: none;
        }
        @keyframes pulse {
            0%, 100% { transform: scale(1); filter: drop-shadow(0 0 20px #4caf50); }
            50% { transform: scale(1.05); filter: drop-shadow(0 0 50px #8bc34a); }
        }
        ...
        .life-label { font-size: 0.8em; color: #81c784; margin-top: 5px; text-align: center; }

        .card {
            background: rgba(10,20,10,0.8); backdrop-filter: blur(18px);
            border: 1px solid rgba(76,175,80,0.3); border-radius: 24px;
            padding: 25px; margin: 20px 0; text-align: center;
            box-shadow: 0 0 30px rgba(76,175,80,0.08);
        }
        .status-badge {
            display: inline-block; background: #1b5e20;
            color: #a5d6a7; padding: 6px 18px; border-radius: 30px;
            font-size: 0.9em; margin: 10px 0;
        }
        .balance {
            font-size: 4em; font-weight: bold; color: #4caf50;
            text-shadow: 0 0 25px #2e7d32; margin: 20px 0; direction: ltr;
        }
        ...
        .globe {
            ...
            background: radial-gradient(circle at 30% 30%, #2e7d32, #0a2a0a);
        ...
        .globe-wave {
            ...
            border: 2px dashed #4caf50; border-radius: 50%;
        ...
        .btn {
            background: rgba(30,50,30,0.7); backdrop-filter: blur(8px);
            border: 1px solid #4caf50; color: #c8e6c9;
        ...
        .btn:hover { background: #2e7d32; border-color: #8bc34a; transform: translateY(-2px); }
        ...
        .fingerprint-scanner {
            ...
            background: rgba(76,175,80,0.1); border: 3px solid #4caf50;
            display: flex; justify-content: center; align-items: center;
        ...
        .log-area {
            background: rgba(5,10,5,0.7); border-radius: 16px;
            padding: 15px; margin-top: 20px; max-height: 180px;
            overflow-y: auto; text-align: right; font-size: 0.9em;
        ...
        .nav-links a { color: #4caf50; text-decoration: none; margin: 0 10px; font-size: 0.9em; }
...
<body>
    ...
    <div class="splash" id="splashScreen">
        <div class="splash-logo">ⴾ</div>
        <p class="splash-text">⏳ جاري الاتصال بالشريحة البيولوجية...</p>
    ...
    <div class="container">
        <div class="hero">
            <div class="logo-main" id="logoIcon">ⴾ</div>
            <h1 class="title">فيتا</h1>
            <p class="subtitle">Vita — المال الحي</p>
        </div>

        <div class="nav-links">
            <a href="index.html">🏠 المحفظة</a> |
            <a href="whitepaper.html">📜 الورقة البيضاء</a> |
            <a href="about.html">ℹ️ عن فيتا</a> |
            <a href="community.html">👥 الكروب</a>
        </div>

        <div class="life-label">🧬 طاقة الحياة (معدل التعدين الحيوي)</div>
        <div class="life-bar-wrap"><div class="life-bar-fill" id="lifeBar"></div></div>
        ...
        <div class="card">
            ...
            <div class="balance" id="balanceDisplay">1,250.50</div>
            <div class="unit">ⴾ Vita في المخزون الحيوي</div>

            <div class="info-line">
                <div class="info-item">
                    <div class="label"><i class="fas fa-wave-square"></i> التردد البيولوجي</div>
                    <div class="value"># 7.83 Hz / ID: 4401</div>
                </div>
                <div class="info-item">
                    <div class="label"><i class="fas fa-heartbeat"></i> إثبات الحياة</div>
                    <div class="value" id="lifeStatus">نشط</div>
                </div>
                <div class="info-item">
                    <div class="label"><i class="fas fa-globe-asia"></i> الموجة الأرضية</div>
                    <div class="value">متصل</div>
                </div>
            </div>

            <div class="btn-group">
                <button class="btn biometric" onclick="openFingerprint()">
                    <i class="fas fa-fingerprint"></i> بصمة فيتا
                </button>
                <button class="btn" onclick="sendVita()">
                    <i class="fas fa-broadcast-tower"></i> إرسال عبر التردد
                </button>
                <button class="btn life" onclick="proofOfLife()">
                    <i class="fas fa-seedling"></i> إثبات الحياة
                </button>
            </div>
        </div>

        <div class="log-area" id="logArea">
            <div class="entry mine"><i class="fas fa-circle"></i> ⚡ أثبت الحياة: +0.31 Vita</div>
            <div class="entry receive"><i class="fas fa-circle"></i> 📡 استُقبل 5.00 Vita من التردد #7823</div>
            <div class="entry send"><i class="fas fa-circle"></i> 📤 أُرسل 2.50 Vita إلى التردد #1190</div>
        </div>

        <div class="footer">
            <i class="fas fa-leaf"></i> Vita ex Terra, Vita in Corpore <i class="fas fa-leaf"></i><br>
            الحياة من الأرض، الحياة في الجسد
        </div>
    </div>

    <div id="fingerprintOverlay" class="fingerprint-overlay" style="display: none;">
        <div class="fingerprint-scanner" id="fingerScanner" onclick="simulateFingerprint()">
            <i class="fas fa-fingerprint"></i>
        </div>
        <p class="fingerprint-text" id="fingerText" style="color:#c8e6c9;">المس الدائرة لمحاكاة البصمة</p>
    </div>
    ...
    <script>
        ...
        function playSchumann() {
            ...
            const o = audioCtx.createOscillator(), g = audioCtx.createGain();
            o.connect(g); g.connect(audioCtx.destination);
            o.type = 'sawtooth'; o.frequency.setValueAtTime(7.83, audioCtx.currentTime);
        ...
        function playScanSound() {
            ...
            o.frequency.linearRampToValueAtTime(1200, audioCtx.currentTime + 0.2);
        ...
        function updateDisplay() {
            ...
            document.getElementById('lifeBar').style.width = lifeEnergy + '%';
        }
        function addLog(msg, type) {
            const la = document.getElementById('logArea');
            const e = document.createElement('div');
            e.className = 'entry ' + type;
            e.innerHTML = '<i class="fas fa-circle"></i> ' + msg;
            la.prepend(e);
        }
        ...
        function openFingerprint() {
            document.getElementById('fingerprintOverlay').style.display = 'flex';
            playScanSound();
        }
        function simulateFingerprint() {
            const s = document.getElementById('fingerScanner');
            s.style.background = 'rgba(76,175,80,0.4)';
            s.style.borderColor = '#8bc34a';
            document.getElementById('fingerText').innerText = '✅ تم تأكيد الهوية البيولوجية';
            playHeartbeat();
            addLog('🔐 تأكيد بصمة بيولوجية: الهوية مطابقة', 'receive');
            setTimeout(() => {
                document.getElementById('fingerprintOverlay').style.display = 'none';
                s.style.background = 'rgba(76,175,80,0.1)';
                s.style.borderColor = '#4caf50';
            }, 1500);
        }
        ...
        function proofOfLife() {
            ...
            balance += g;
            lifeEnergy = Math.min(100, lifeEnergy + 3);
            updateDisplay();
            document.getElementById('lifeStatus').innerText = 'نبض حيوي';
            playHeartbeat();
            addLog('⚡ أثبت الحياة: +' + g.toFixed(4) + ' Vita', 'mine');
            setTimeout(() => { document.getElementById('lifeStatus').innerText = 'نشط'; }, 1200);
        }
        ...
        function sendVita() {
            ...
            if (balance >= a) {
                balance -= a;
                updateDisplay();
                const tf = Math.floor(Math.random() * 9000 + 1000);
                playSchumann();
                addLog('📤 أُرسل ' + a.toFixed(2) + ' Vita إلى التردد #' + tf + ' عبر الموجة الأرضية', 'send');
            } else {
                addLog('⚠️ المخزون الحيوي غير كافٍ', 'send');
            }
        }
        ...
        setInterval(() => {
            if (Math.random() < 0.15) {
                ...
                balance += a;
                updateDisplay();
                const sf = Math.floor(Math.random() * 9000 + 1000);
                addLog('📡 استُقبل ' + a.toFixed(2) + ' Vita من التردد #' + sf, 'receive');
            }
        }, 15000);

        document.getElementById('logoIcon').addEventListener('click', () => {
            playSchumann();
            addLog('🌐 بث نبضة حيوية عبر الموجة الأرضية', 'mine');
        });
    </script>
</body>
</html>"""
...
<body>
    <p><a href="index.html">← العودة للمحفظة</a></p>
    <h1>ⴾ فيتا (Vita)</h1>
    ...
    <p>تكافئ فيتا النشاط الحيوي: كل خطوة، نبضة، ونفَس يُنتج وحدات من العملة. النوم يوقف التعدين مؤقتًا.</p>
    
    <h2>6. الأمان</h2>
    <p>لا يمكن اختراق فيتا إلكترونيًا (لا إنترنت)، ولا يمكن تزويرها (التردد البروتيني فريد)، ولا يمكن مصادرتها (هي جزء من جسدك).</p>
    
    <p style="font-size:1.5em; text-align:center; margin-top:40px;">Vita ex Terra, Vita in Corpore</p>
    <p style="text-align:center; color:#4caf50;">ⴾ</p>
</body>
</html>"""
...
<body>
    ...
    <p>"Vita ex Terra, Vita in Corpore"</p>
...
def main():
    print("=" * 50)
    print(" ⴾ  إنشاء ملفات مشروع فيتا...")
    print("=" * 50)
    
    # إنشاء الملفات مباشرة في المجلد الحالي
    files = {
        "index.html": get_index_html(),
        "whitepaper.html": get_whitepaper_html(),
        "about.html": get_about_html(),
        "community.html": get_community_html(),
        "README.md": get_readme_md(),
    }
    ...
    print()
    print("✅ تم إنشاء جميع ملفات فيتا في الجذر!")
    print("   افتح index.html في متصفحك لرؤية المحفظة")
